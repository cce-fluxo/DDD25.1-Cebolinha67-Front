...
export default function VoltarContinuarButton({ habilitado }: VoltarContinuarButtonProps) {
  ...
  return (
    <div className="flex gap-3 mt-8 md:mt-12 justify-center w-full">
      <button 
        className="bg-indigo-200 text-black px-6 py-2 rounded-xl font-medium" 
        onClick={() => router.back()}
      >
        Voltar
      </button>
      <button 
        className={`px-10 py-2 rounded-xl font-medium text-white transition-opacity ${habilitado ? "bg-indigo-500" : "bg-indigo-300 cursor-not-allowed"}`}
